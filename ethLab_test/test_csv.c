...
#define TASK_FREQUENCY (2 * 1000)
#define WORK_MODE 9
#define MAX_TARGET_VEL 1000000
...
#define SLV1 0, 1
#define SLV2 0, 2
#define VENDOR_PRODUCT 0x000116c7, 0x003e0402
...
const static ec_pdo_entry_reg_t domain1_regs[] = {
    ...
    {SLV0, VENDOR_PRODUCT, 0x60FF, 0, &output[0].targetVelocity, NULL},
    ...
    {SLV0, VENDOR_PRODUCT, 0x606C, 0, &input[0].actualVelocity, NULL},

    {SLV1, VENDOR_PRODUCT, 0x6040, 0, &output[1].controlWord, NULL},
    {SLV1, VENDOR_PRODUCT, 0x6060, 0, &output[1].workModeOut, NULL},
    {SLV1, VENDOR_PRODUCT, 0x60FF, 0, &output[1].targetVelocity, NULL},
    ...
    {SLV1, VENDOR_PRODUCT, 0x606C, 0, &input[1].actualVelocity, NULL},

    {SLV2, VENDOR_PRODUCT, 0x6040, 0, &output[2].controlWord, NULL},
    {SLV2, VENDOR_PRODUCT, 0x6060, 0, &output[2].workModeOut, NULL},
    {SLV2, VENDOR_PRODUCT, 0x60FF, 0, &output[2].targetVelocity, NULL},
    ...
    {SLV2, VENDOR_PRODUCT, 0x606C, 0, &input[2].actualVelocity, NULL},
    {}
};
...
static ec_pdo_info_t device_pdos[] = {
    ...
    {0x1A00, 10, device_pdo_entries + 4},
};
...
"<!DOCTYPE html><html lang=\"zh\"><head><meta charset=\"utf-8\"><title>CSV速度模式</title>"
...
"<div><label>速度</label><input id=\"step\" type=\"number\" min=\"1\" value=\"20000\"></div>"
...
static void handle_http(int cfd, const char *req, int req_len) {
    ...
    if (strncmp(req, "GET ", 4) == 0) {
        ...
        if (plen == 1 && path[0] == '/') { http_send(cfd, "200 OK", "text/html", UI_HTML); return; }
        if (strncmp(path, "/status", plen) == 0) {
            ...
            snprintf(buf, sizeof(buf), "{\"run\":%s,\"dir\":%d,\"vel\":%d,\"pos0\":%d,\"pos1\":%d,\"pos2\":%d}", run?"true":"false", dir, step, g_last_actual_pos[0], g_last_actual_pos[1], g_last_actual_pos[2]);
        ...
        if (strncmp(path, "/diag", plen) == 0) {
            ...
            int32_t tgtv0 = EC_READ_S32(domain1_pd + output[0].targetVelocity);
            int32_t tgtv1 = EC_READ_S32(domain1_pd + output[1].targetVelocity);
            int32_t tgtv2 = EC_READ_S32(domain1_pd + output[2].targetVelocity);
            ...
            int32_t vel0 = EC_READ_S32(domain1_pd + input[0].actualVelocity);
            int32_t vel1 = EC_READ_S32(domain1_pd + input[1].actualVelocity);
            int32_t vel2 = EC_READ_S32(domain1_pd + input[2].actualVelocity);
            pthread_mutex_lock(&cmd_mutex);
            bool run = g_cmd.run; int dir = g_cmd.dir; int step = g_cmd.step;
            pthread_mutex_unlock(&cmd_mutex);
            int ack0 = (sw0 & 0x1000) ? 1 : 0;
            int ack1 = (sw1 & 0x1000) ? 1 : 0;
            int ack2 = (sw2 & 0x1000) ? 1 : 0;
            int trg0 = (sw0 & 0x0400) ? 1 : 0;
            int trg1 = (sw1 & 0x0400) ? 1 : 0;
            int trg2 = (sw2 & 0x0400) ? 1 : 0;
            snprintf(buf, sizeof(buf),
                     "{\"status\":[%u,%u,%u],\"mode\":[%d,%d,%d],\"followingErr\":[%d,%d,%d],\"err\":[%u,%u,%u],\"servoErr\":[%u,%u,%u],\"din\":[%u,%u,%u],\"tpst\":[%u,%u,%u],\"tpp\":[%d,%d,%d],\"tgtv\":[%d,%d,%d],\"vel\":[%d,%d,%d],\"act\":[%d,%d,%d],\"ack12\":[%d,%d,%d],\"targetReached10\":[%d,%d,%d],\"cmd\":{\"run\":%s,\"dir\":%d,\"vel\":%d}}",
                     sw0, sw1, sw2, md0, md1, md2, fe0, fe1, fe2, ec0, ec1, ec2, sec0, sec1, sec2, di0, di1, di2,
                     tpst0, tpst1, tpst2, tpp0, tpp1, tpp2,
                     tgtv0, tgtv1, tgtv2,
                     vel0, vel1, vel2,
                     act0, act1, act2,
                     ack0, ack1, ack2,
...
void cyclic_task() {
    static uint32_t time_cnt[3] = {0, 0, 0}; static bool servo_enabled[3] = {false, false, false}; static int debug_counter = 0; static int sp_toggle[3] = {0, 0, 0};
    ...
    for (int i = 0; i < 3; ++i) {
        uint16_t status_i = EC_READ_U16(domain1_pd + input[i].statusword); uint16_t control_i = 0;
        if (!servo_enabled[i]) {
            ...
            switch (status_i & 0x6F) {
                ...
                case 0x21: control_i = 0x07; EC_WRITE_S32(domain1_pd + output[i].targetVelocity, 0); break;
                case 0x23: control_i = 0x0F; break;
                case 0x27: control_i = 0x0F; servo_enabled[i] = true; sp_toggle[i] = 0; break;
            ...
            EC_WRITE_U16(domain1_pd + output[i].controlWord, control_i); EC_WRITE_S8(domain1_pd + output[i].workModeOut, WORK_MODE);
        } else {
            time_cnt[i]++;
            if (time_cnt[i] == 1) {
                EC_WRITE_S32(domain1_pd + output[i].targetVelocity, 0);
                uint16_t cw = 0x0F | 0x10;
                EC_WRITE_U16(domain1_pd + output[i].controlWord, cw);
                sp_toggle[i] = 1;
            } else {
                pthread_mutex_lock(&cmd_mutex); int dir = g_cmd.dir; int step = g_cmd.step; bool run = g_cmd.run; pthread_mutex_unlock(&cmd_mutex);
                int vel = (run ? (dir * step) : 0);
                if (vel > MAX_TARGET_VEL) vel = MAX_TARGET_VEL;
                if (vel < -MAX_TARGET_VEL) vel = -MAX_TARGET_VEL;
                EC_WRITE_S32(domain1_pd + output[i].targetVelocity, vel);
                uint16_t cw = 0x0F | (sp_toggle[i] ? 0x00 : 0x10);
                EC_WRITE_U16(domain1_pd + output[i].controlWord, cw);
                sp_toggle[i] ^= 1;
            }
            ...
            if (time_cnt[i] % 500 == 0) {
                ...
                int32_t actual_vel_i = EC_READ_S32(domain1_pd + input[i].actualVelocity);
                ...
                printf("[V%d] tgtv:%d vel:%d pos:%d sw:0x%04X mode:%d fe:%d err:0x%04X servoErr:0x%04X din:0x%08X tpst:0x%04X tpp:%d\n",
                       i, EC_READ_S32(domain1_pd + output[i].targetVelocity), actual_vel_i, actual_pos_i, sw_i, md_i, fe_i, ec_i, sec_i, di_i, tpst_i, tpp_i);
...
int main(int argc, char **argv) {
    printf("Starting three-slave CSV motor controller\n"); master = ecrt_request_master(0); if (!master) exit(EXIT_FAILURE);
    domain1 = ecrt_master_create_domain(master); if (!domain1) exit(EXIT_FAILURE);
    ...
    while (1) { usleep(TASK_FREQUENCY); cyclic_task(); }
    return EXIT_SUCCESS;
}
